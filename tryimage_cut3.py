...
for mm in range(NUM_PIC):
    ...
    paintx = np.zeros(coll[mm].shape, np.uint8)
    ...
    w = [0] * coll[mm].shape[1]
    # 对每一行计算投影值
    for x in range(coll[mm].shape[1]):
    ...
    x = np.arange(1, coll[mm].shape[1] + 1, 1)
    y = np.array(w)
    # 第一个拟合，自由度为3
    ...
    for i in range(root.size):
        if root[i].imag == 0 and root[i].real > 0 and root[i] < coll[mm].shape[1]:
            root1.append(root[i])
            j = j + 1
    print(root1)
    middle = coll[mm].shape[1] / 2
    for i in range(j):
